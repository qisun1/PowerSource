...
void setup() {
	...
	moveCursor(0);
	brightnessLevel = 100;
	pinMode(brightnessPIN, OUTPUT);   // sets the pin as output
...
void moveCursor(int8_t moveNext)
{
	...
	else if (menuLevel == 2)
	{
		...
		{
			PotDCVoltage.Up();
			DCVoltage = PotDCVoltage.GetEstimatedResistance();
		}
		else
		{
			PotDCVoltage.Down();
	...
	else if (menuLevel == 4)
	{
		if (moveNext > 0)
		{		
			if (brightnessLevel <= 250- brightnessStep)
			{
				brightnessLevel += brightnessStep;
			}				
		}
		else
		{
			if (brightnessLevel >= brightnessStep)
			{
				brightnessLevel -= brightnessStep;
			}
		}
